...
\caption{Écran \textsc{instrument}}
\label{fig:instr}
\end{figure}
...
Le champ \textsc{type} définit le type d’instrument. Les types sont propres à chaque canal~:
les instruments \textsc{pulse} ne fonctionnent que dans les canaux Pulse, les instruments \textsc{wave} et \textsc{kit}
dans le canal Wave, et les instruments \textsc{noise} dans le canal Noise.

Essayons maintenant les kits de percussions samplés. Pour cela, nous devons utiliser le canal Wave.
Revenez à l’écran \textsc{song}, placez le curseur sur le canal Wave, puis créez une nouvelle chaîne et une nouvelle phrase en appuyant sur \textsc{a}.
Insérez une note avec \textsc{a}, puis \textsc{select+right} pour éditer l’instrument.
...
Voici une table de conversion entre les deux systèmes~:
...
Notez bien que les valeurs décimales et hexadécimales sont identiques ; seule la représentation change.
L’intérêt principal de l’hexadécimal est de gagner de la place à l’écran~: avec des nombres hexadécimaux, on peut représenter toutes les valeurs de 0 à 255 avec seulement deux chiffres, de 00 à FF.

Représenter des nombres négatifs sur deux chiffres peut être délicat.
Dans Little Sound Dj, les nombres sont cycliques. Cela signifie que si l’on soustrait 1 au plus petit nombre possible (0), on obtient la plus grande valeur possible (FF).
...
\begin{itemize}
    \item Pour éviter toute perte de données, utilisez toujours des piles neuves. Remplacez-les dès que la LED rouge de votre Game Boy faiblit ou que l’écran s’assombrit.
    ...
    \item Assurez-vous que la cartouche est bien enfoncée. Un morceau de ruban adhésif peut parfois aider à la caler fermement.
    \item Réinitialisez complètement la mémoire de la cartouche. Pour ce faire, maintenez \textsc{select+a+b} sur le bouton \textsc{load/save file} de l’écran \textsc{project}.
